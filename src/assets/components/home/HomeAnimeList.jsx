...
export default function AnimeList({}) {
  ...
  useEffect(() => {
    const getAPI = async () => {
      try {
        const response = await fetch(seasonNowAnime(currentPage));
        if (!response.ok) {
        ...
        setAnimes((prevAnimes) => {
          const existingMalIds = new Set(
            prevAnimes.map((anime) => anime.mal_id)
          );
          const newMalIds = new Set();
          const uniqueNewAnimes = filteredAnimes.filter((anime) => {
            if (
              existingMalIds.has(anime.mal_id) ||
              newMalIds.has(anime.mal_id)
            ) {
              return false;
            } else {
  ...
  return (
    <div className="home__anime__list container mt-16" ref={outElement}>
      {loading || currentPost.length < 20 ? (
        ...
        <>
          ...
          <div className="anime__box-list box__list rounded-b" ref={animeBox}>
            <ul className="grid">
